...
use poise::{
    serenity_prelude::{
        self as serenity, Cache, CacheHttp, CreateEmbed, EditMessage, Http, Message, MessageId,
    },
...
use crate::{
    db::queries::config::LogChannel, BotData, ChannelId, Context, GuildId, GAGBOT_COLOR_ERROR,
    GAGBOT_COLOR_LOG_DELETE, GAGBOT_COLOR_LOG_EDIT, GAGBOT_COLOR_LOG_JOIN, GAGBOT_COLOR_LOG_LEAVE,
    GAGBOT_COLOR_NORMAL, GAGBOT_COLOR_SUCCESS, GAGBOT_ICON, GAGBOT_ICON_ERROR, commands::promote::OptionallyConfiguredResult,
};
...
impl Embed {
    ...
    pub fn random_color(mut self) -> Self {
        ...
        self.color = Some(Color::from_rgb(color[0], color[1], color[2]));
        self
    }
    ...
    pub async fn send<'a>(mut self, ctx: &Context<'a>) -> Result<ReplyHandle<'a>, serenity::Error> {
    ...
    pub async fn send_in_channel<'a>(
        ...
        ctx: impl AsRef<Http>,
    ) -> Result<OptionalMessage, serenity::Error> {
        ...
        channel_id
            ...
            .await
            .map(|m| OptionalMessage::Message(m))
    }
...
impl OptionalMessage {
    pub fn is_some(&self) -> bool {
        match self {
            OptionalMessage::Message(_) => true,
            _ => false
        }
    }

    pub async fn edit<'a, F>(
        &mut self,
        cache_http: impl CacheHttp,
        f: F,
    ) -> Result<(), serenity::SerenityError>
    where
        F: for<'b> FnOnce(&'b mut EditMessage<'a>) -> &'b mut EditMessage<'a>,
    {
        match self {
            Self::Message(message) => message.edit(cache_http, f).await,
            _ => Ok(()),
        }
    }
}

/// Creates an embed in the given log channel if it is configured, updates it
/// for progress given by the provided function and updates it with an error if
/// the function returns Err
/// 
/// If existing_message is supplied it will be used instead of creating a new
/// message. This is so you can create a response in the channel the command was
/// executed in and/or use ephemeral messages
///
/// Errors generated by this function are only logged with error!() so there's
/// no feedback to the discord user if there is a DB error or similar
pub async fn with_progress_embed<'a, F, Fut, T, R, E, C, Ctx>(
    ...
    f: F,
    work_context: C,
) -> Result<OptionallyConfiguredResult<R>, E>
where
    ...
    T: ToString,
    Fut: 'a + Send + Future<Output = Result<OptionallyConfiguredResult<R>, E>>,
    E: 'static + Display + Send,
    R: 'static + Send,
{
    let title = title.to_string();

    let log_channel_id = match data.log_channel(guild_id, vec![log_kind]).await {
        Ok(r) => r,
        Err(e) => {
    ...
    macro_rules! base_embed {
        () => {
            Embed::default().title(&title)
        };
    }

    let mut progress_message = if let Some(log_channel_id) = log_channel_id {
        match base_embed!().send_in_channel(log_channel_id, ctx).await {
            Ok(m) => m,
            Err(e) => {
    ...
    match r.as_ref() {
        Ok(OptionallyConfiguredResult::Ok(_)) => update_embed!(Some(":white_check_mark: Completed".to_string()), EmbedFlavour::Success),
        Ok(OptionallyConfiguredResult::Unconfigured(key)) => 
            update_embed!(Some(format!(":grey_question: Not configured: {key}")), EmbedFlavour::Normal),
        Err(e) => update_embed!(Some(format!(":x: {}", e)), EmbedFlavour::Error),
    }
